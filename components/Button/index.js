...
export default function Button({ children, type, to }) {
  return (
    <Link href={to}>
      <a>
        {type === "primary" ? (
          <button className="bg-indigo-500 hover:bg-indigo-600 dark:bg-blue-600 dark:hover:bg-blue-500 sm:mr-8 text-white font-bold w-36 py-2 px-4 rounded transition-colors">
            {children}
          </button>
        ) : (
